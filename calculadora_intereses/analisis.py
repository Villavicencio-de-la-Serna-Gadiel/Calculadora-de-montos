...
def analisis(tab2):
    def mostrar_graficos(diccionario):
        ...
        with grafico_lineas:
            st.line_chart(dt, x = "Monto", y = "Periodo")
        with grafico_barras:
            st.bar_chart(dt, x = "Monto", y = "Periodo")
        with grafico_areas:
            st.area_chart(dt, x = "Monto", y = "Periodo")
        ...
        with st.expander("Filtros"):
            ...
            with st.form("Filtro"):
                ...
                st.form_submit_button("Registrar",
                                      on_click = registrar_filtros,
                                      key="registrar")
            st.button("Quitar filtros",
                      on_click = quitar_filtros,
                      key = "quitar_filtros")
    with tab2:
        if "letra" in st.session_state:
            ...
            st.image("calculadora_intereses/grafico.svg",
                     caption = '''Debe agregarse una letra en la barra lateral para que
